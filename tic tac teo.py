...
for row in range(3):
    for column in range (3):
        board[row][column] = tkinter.Button(
            frame,
            text="",
            font=("consolas", 50, "bold"),  # Fixed typo
            background=color_gray,
            foreground=color_blue,
            width=4,
            height=1,
            command=lambda row=row, column=column: set_tile(row, column)  # Ensure set_tile is defined
        )
...
#format "(w)x(h)+(x)+(y)"
window.geometry(f"{window_x}x{window_height}+{window_x}+{window_y}")
